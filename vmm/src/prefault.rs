...
// Give each region a thread count in proportion to its size, bounded so that
// no node's regions exceed its cores and the total does not exceed the
// machine. Sharing a pool's cores by size keeps each node within budget, and
// the final scale keeps the total within the machine, which only bites when
// pinned and unpinned regions run together.
fn assign_threads(plans: &mut [RegionPlan], machine_cores: usize) {
    for i in 0..plans.len() {
        if plans[i].num_pages == 0 {
            continue;
        }
        let region_bytes = plans[i].bytes();
...
pub(crate) fn prefault_regions(regions: &[PrefaultRegion]) -> Result<(), Error> {
    ...
    // Opened once every worker has spawned so none begins populating
    // while another's stack mmap is still queued as a writer. false
    // means a spawn failed, so the workers that started exit.
    ...
    thread::scope(|s| -> Result<(), Error> {
        ...
        for range in &ranges {
            ...
            let worker = move || -> Result<(), io::Error> {
                // Stand down if a sibling failed to spawn.
                if !*start.wait() {
                    return Ok(());
                }
            ...
            match thread::Builder::new().spawn_scoped(s, worker) {
                ...
                Err(e) => {
                    // Abort the workers that already spawned, then fail.
        ...
        // Every worker exists now, release them together.
