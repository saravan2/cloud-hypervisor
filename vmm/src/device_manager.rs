...
impl DeviceManager {
    ...
    pub fn remove_device(&mut self, id: &str) -> DeviceManagerResult<()> {
        ...
        if let PciDeviceHandle::Virtio(virtio_pci_device) = pci_device_handle {
            ...
            // Mirror the virtio-net cleanup above for externally-provided
            // VFIO cdev FDs: remove the preserved original from VmConfig
            // and close it. The VfioDevice owns a dup which is already
            // closed by its Drop.
            let mut config = self.config.lock().unwrap();
